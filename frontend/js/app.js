const app = (() => {
      ...
      link.addEventListener("click", (e) => {
        ...
        [
          ...
          document
            ...
            .classList.toggle("hidden", view !== v);
        });
        const titles = {
          dashboard: "Dashboard",
          entries: "Entries",
  ...
  async function fetchAndRenderEntries() {
    try {
      ...
      const [page, full] = await Promise.all([
        api.getEntriesPaginated({
          ...filters,
          limit: ENTRIES_PAGE_SIZE,
          offset: (entriesPage - 1) * ENTRIES_PAGE_SIZE,
        }),
  ...
  // No polling here unlike Entries/Meetings/Users — this is a point-in-time
  // review surface a manager opens deliberately, not something that needs
  // to stay live-updated while sitting open in a background tab.

  async function loadAuditLog() {
    auditLogPage = 1;
    await fetchAndRenderAuditLog();
  }

  async function goToAuditLogPage(page) {
    auditLogPage = page;
    await fetchAndRenderAuditLog();
  }
  ...
  async function fetchAndRenderAuditLog() {
    try {
      ...
      const page = await api.getAuditLog({
        ...
        offset: (auditLogPage - 1) * AUDIT_LOG_PAGE_SIZE,
      });
      audit.renderTable(page.data, "audit-log-container");
      renderAuditLogPagination(page.total);
    } catch (err) {
      console.error(err);
    }
  }

  function renderAuditLogPagination(total) {
    ...
    const start = (auditLogPage - 1) * AUDIT_LOG_PAGE_SIZE + 1;
    const end = Math.min(auditLogPage * AUDIT_LOG_PAGE_SIZE, total);
    el.innerHTML = `
      <span class="pagination-info">${start}–${end} of ${total}</span>
      <div class="pagination-controls">
        <button class="btn" ${auditLogPage <= 1 ? "disabled" : ""} onclick="app.goToAuditLogPage(${auditLogPage - 1})">‹ Prev</button>
        <span class="pagination-page">Page ${auditLogPage} of ${totalPages}</span>
        <button class="btn" ${auditLogPage >= totalPages ? "disabled" : ""} onclick="app.goToAuditLogPage(${auditLogPage + 1})">Next ›</button>
      </div>`;
  }
  ...
  function renderEntriesSummary(data) {
    ...
    const catHtml = Object.entries(byCategory)
      .map(
        ([cat, h]) => `<span>${cat}: <strong>${h.toFixed(1)}h</strong></span>`,
      )
      .join("");
    const otHtml = overtime
      ? `<span style="color:var(--danger)">⚠️ ${overtime} overtime</span>`
      : "";
  ...
  async function addEntry() {
    ...
    if (
      hours > 8 &&
      !confirm(`⚠️ You're logging ${hours}h which exceeds 8h. Continue?`)
    )
      return;
    try {
  ...
  async function refreshEntriesResilient(includeEntriesList = false) {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Could not refresh entries automatically — it will catch up on the next action.",
        );
      }
    }
  }
  ...
  async function saveEdit() {
    ...
    if (!data.date || !data.hours || !data.activity) {
      alert("Please fill in all fields.");
      return;
    }
    try {
  ...
  async function restoreTimer() {
    try {
      ...
      console.error("Timer restore error");
    }
  }
  ...
  function getReportRange() {
    ...
    if (reportPeriod === "week") {
      const day = d.getDay();
      const start = new Date(d);
      start.setDate(d.getDate() - (day === 0 ? 6 : day - 1) + reportOffset * 7);
      const end = new Date(start);
      end.setDate(start.getDate() + 6);
  ...
  async function renderReport() {
    ...
    document.getElementById("report-period-label").textContent = formatLabel(
      from,
      to,
    );
          ...
          <table class="cat-table">
            ...
            <tbody>
              ${r.days
                ...
                ${
                  d.user_breakdown && d.user_breakdown.length > 0
                    ? d.user_breakdown
                        .map(
                          (u) => `
    ...
    if (!r.entries.length) {
      document.getElementById("report-container").innerHTML =
        '<div class="report-empty">No entries for this period.</div>';
      return;
    }

    const grouped = r.entries.reduce((acc, e) => {
    ...
    document.getElementById("report-container").innerHTML = `
      ...
      ${Object.keys(grouped)
        ...
        .map((date) => {
          ...
          const userTotals = dayEntries.reduce((acc, e) => {
            acc[e.user_id] = (acc[e.user_id] || 0) + e.hours;
            return acc;
          }, {});
  ...
  async function deleteUser(userId, username) {
    if (
      !confirm(
        `Delete user "${username}"? This will also delete all their entries and clock sessions.`,
      )
    )
      return;
    try {
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-top:1rem;">
          <tbody>
            ...
            `,
                    ...
                    .join("")
                : `<tr><td style="padding:12px 0;color:var(--text-muted);">No teams configured yet.</td></tr>`
            }
          </tbody>
        </table>`;
    } catch {
  ...
  async function deleteTeam(teamId, name) {
    if (
      !confirm(
        `Delete team "${name}"? Members will be unassigned, not deleted.`,
      )
    )
      return;
    try {
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;margin-top:1rem;">
          ...
          <tbody>
            ...
            `,
                    ...
                    .join("")
                : `<tr><td colspan="5" style="padding:12px 0;color:var(--text-muted);">No rooms configured yet.</td></tr>`
            }
          </tbody>
        </table>
  ...
  async function createRoom() {
    ...
    const equipment = document
      .getElementById("new-room-equipment")
      .value.trim();
  ...
  async function saveRoom(roomId) {
    const capacityRaw = document.getElementById(
      `room-capacity-${roomId}`,
    ).value;
    const equipment = document
      .getElementById(`room-equipment-${roomId}`)
      .value.trim();
  ...
  async function loadClockSessions() {
    try {
      ...
      alert("Failed to load clock sessions.");
    }
  }
  ...
  async function saveClockSession(id) {
    ...
    try {
      ...
      alert(err.message || "Failed to update clock session.");
    }
  }
  ...
  function populateRoomOptions() {
    ...
    if (!datalist) return;
    datalist.innerHTML = roomsList
      .filter((r) => r.status !== "renovation")
      .map((r) => {
        const label = [
          r.capacity ? `${r.capacity} seats` : null,
          r.equipment || null,
        ]
          .filter(Boolean)
          .join(" · ");
  ...
  function populateAvailabilityRoomSelect() {
    ...
    select.innerHTML =
      ...
      roomsList
        ...
        .join("");
    if (previous && roomsList.some((r) => String(r.id) === previous))
  ...
  async function loadMeetingsRoomAvailability(roomId) {
    ...
    try {
      ...
      listEl.innerHTML = `<p class="card-subtitle" style="color:var(--danger);">Failed to load this room's schedule.</p>`;
    }
  }
  ...
  function renderSidebarForSelectedDate() {
    const dayMeetings = monthMeetings.filter(
      (m) => m.date === selectedCalendarDate,
    );
    const label =
      selectedCalendarDate === today()
        ? "Today"
        : meetings.formatDate(selectedCalendarDate);
  ...
  function onMeetingSearchInput(value) {
    ...
    meetingSearchDebounce = setTimeout(async () => {
      try {
        ...
        /* leave previous results visible on transient failure */
      }
    }, 300);
  }
  ...
  async function declineMeeting(id) {
    const reason = prompt(
      "Optional — let the organizer know why (leave blank to skip):",
    );
  ...
  function toggleMeetingLocation() {
    const isInPerson =
      document.querySelector('input[name="meeting-location-type"]:checked')
        .value === "in_person";
    document
      .getElementById("meeting-room-group")
      .classList.toggle("hidden", !isInPerson);
    document
      .getElementById("meeting-link-group")
      .classList.toggle("hidden", isInPerson);
  }

  function toggleGoogleMeetOption() {
    const useGoogleMeet = document.getElementById(
      "meeting-use-google-meet",
    ).checked;
    document
      .getElementById("meeting-link")
      .classList.toggle("hidden", useGoogleMeet);
    document
      .getElementById("meeting-link-label")
      .classList.toggle("hidden", useGoogleMeet);
  ...
  function toggleRecurrence() {
    ...
    document
      .getElementById("meeting-recurrence-until-group")
      .classList.toggle("hidden", !repeats);
  }
  ...
  async function scheduleMeeting() {
    ...
    const description = document
      .getElementById("meeting-description")
      .value.trim();
    const location_type = document.querySelector(
      'input[name="meeting-location-type"]:checked',
    ).value;
    ...
    const use_google_meet = document.getElementById(
      "meeting-use-google-meet",
    ).checked;
    const attendee_ids = meetings.getSelectedAttendees();
    const recurrence = document.getElementById("meeting-recurrence").value;
    const recurrence_until = document.getElementById(
      "meeting-recurrence-until",
    ).value;
    ...
    try {
      result = await api.createMeeting({
        ...
        recurrence,
        recurrence_until: recurrence !== "none" ? recurrence_until : null,
      });
    } catch (err) {
    ...
    if (result.attendee_conflicts && result.attendee_conflicts.length) {
      const lines = result.attendee_conflicts.map(
        (c) =>
    ...
    conflictCheckDebounce = setTimeout(async () => {
      ...
      try {
        const result = await api.checkMeetingConflicts({
          ...
          start_time,
          end_time,
          attendee_ids: attendee_ids.join(","),
        });
        renderAttendeeConflictWarning(result.conflicts);
      } catch {
        /* live check is a courtesy, not critical — fail quietly */
      }
    }, 250);
  }
  ...
  function renderAttendeeConflictWarning(conflicts) {
    ...
    el.innerHTML = `<div class="conflict-warning">⚠️ ${conflicts
      .map(
        (c) =>
  ...
  async function refreshCalendarResilient() {
    try {
      ...
      try {
        ...
        console.warn(
          "Could not refresh the calendar automatically — it will catch up on the next action.",
        );
      }
    }
  }
  ...
  function openRescheduleForm(meetingId) {
    ...
    if (!meeting) {
      alert("Could not find that meeting — try reopening the day.");
      return;
    }
    document.getElementById("reschedule-meeting-id").value = meeting.id;
    document.getElementById("reschedule-title-label").textContent =
      meeting.title;
